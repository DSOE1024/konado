...
畫面會被分成若干等份（支援 2–5 份，預設分 5 份）。從左到右數，角色可放在 `0/5` 到 `5/5` 之間，例如 `1/5` 表示角色位於畫面左側位置。
...
元件會將承載角色的 `Control` 節點畫布容器橫向等分為 `division` 個區塊（區塊數量限制在 2–5），區塊索引從**左到右遞增**（0 為最左側，`division` 為最右側）。

`character_position`（角色位置索引）會限制在 `[0, division]`，角色的**中心**會對齊對應分割線。為避免立繪貼邊造成視覺溢出，通常建議使用 `1` 到 `division - 1`。
