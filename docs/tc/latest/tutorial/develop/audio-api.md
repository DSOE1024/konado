...
`KND_AudioInterface` 提供背景音樂、音效和語音的執行階段播放介面，適用於需要在 GDScript 中直接控制音訊的客製化場景。KonadoScript 中的 `play bgm`、`play sfx` 和 `stop bgm` 等劇情指令仍請參閱「音訊」章節。

## 取得音訊介面

在腳本中匯出 `KND_AudioInterface` 參照，然後在 Inspector 中將對話場景的 `KonadoAudioInterface` 節點指派給它。音訊資源使用 Godot 的 `AudioStream`：
...
對話場景中的音訊介面需要正確設定 `bgm_player`、`sound_effect_player` 和 `voice_player`。呼叫缺少對應播放器的介面會在 Godot 輸出中回報錯誤。

## 背景音樂

`play_bgm()` 會立即開始循環播放背景音樂，不會等待播放結束。再次呼叫會取代目前的背景音樂：
...
- `play_bgm()`、`play_sound_effect()` 和 `play_voice()` 均為非阻塞介面。
