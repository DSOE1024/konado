...
var shot = i18n?.LoadLocalizedScript("res://dialogues/chapter.ks", "zh_Hant");
if (shot != null)
...
| `DialogueLineStart(string nodeId)` | 綁定 `dialogue_line_start(node_id)`。目前版本使用節點 ID，而不是舊版行號。 |
...
Wrapper 類別是 Konado GDScript 資源的輕量 C# 封裝。包裝已有資源時會驗證腳本是否匹配；新建包裝物件時會建立底層 GDScript 資源。只有 Godot API 明確要求底層 `Resource` 時才使用 `SourceResource`；`SetShot(KndShot)` 可直接傳入包裝物件。
...
| `ActorPosition` | `Vector2` | 角色位置。目前版本使用網格定位。 |
