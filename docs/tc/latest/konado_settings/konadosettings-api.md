...
#### `Type`
- **SLIDER (0)**：滑桿類型，用於數值調整。
- **TOGGLE (1)**：開關類型，用於布林值。
- **OPTION (2)**：選項類型，用於下拉選擇。

### 屬性

- **key: String**：設定項目的唯一識別符。
- **label: String**：設定項目的顯示標籤。
- **type: Type**：設定項目的類型。
- **min_value: float**：滑桿最小值，僅 SLIDER 類型使用。
- **max_value: float**：滑桿最大值，僅 SLIDER 類型使用。
- **step: float**：滑桿步長，僅 SLIDER 類型使用。
- **options: Array[String]**：選項清單，僅 OPTION 類型使用。
...
設定面板是一個可視化介面，用於顯示和管理設定。

### 屬性

- **_tab_container: TabContainer**：分頁容器，用於顯示不同分類的設定。
- **btn_reset: Button**：還原預設值按鈕。
...
- **btn_reset.pressed**：點擊還原預設值按鈕時發出。
- **btn_close.pressed**：點擊關閉按鈕時發出。

### 範例

```gdscript
# 載入設定面板場景
var settings_panel = preload("res://addons/konado_settings/scenes/settings_panel.tscn").instantiate()
...
設定系統會自動偵測目前執行平台，並根據設定項目的 `platforms` 屬性過濾顯示內容。
...
設定值會使用 Godot 的 `ConfigFile` 格式，自動儲存到 `user://knd_settings.cfg` 檔案中。
...
- 所有 API 呼叫都應在遊戲啟動後進行。
- 註冊新分類後，需要呼叫 `rebuild()` 方法更新設定面板。
- 平台過濾只影響設定項目的顯示，不影響設定值的儲存。
- 設定項目的 `key` 在分類內必須唯一。
- 對於選項類型，預設值必須存在於 `options` 清單中。
...
- 平台過濾會在取得分類時進行，不會影響設定值的存取效能。
- UI 會依需求建立，只在顯示設定面板時建立控制項。

## 擴充建議

1. **新增設定類型**：
   - 在 `SettingItem.Type` 中新增列舉值。
   - 在 `UIFactory` 中新增對應的建立邏輯。
