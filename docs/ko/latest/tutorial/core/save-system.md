...
- **variables** - 게임 변수
- **audio_state** - 현재 BGM, 재생 위치 및 볼륨 상태
- **actor_state** - 현재 액터, 상태, 위치 및 표시 상태
- **background_state** - 현재 배경 씬과 전환 상태
- **save_time** - 저장 시간
- **version** - 저장 버전

## 저장 파일 형식

저장 파일은 JSON 형식으로 `user://konado_saves/`에 저장되며 이름은 `[슬롯ID].kns`입니다. 기본 설정은 20개 슬롯을 제공하며 유효한 ID는 `0`부터 `19`까지입니다.
