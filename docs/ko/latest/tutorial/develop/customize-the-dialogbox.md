...
사용자 지정 UI는 용도에 맞는 범위에 배치하세요. 시스템 오류 메시지를 덮어야 하는 경우가 아니라면 `120` 이상을 사용하지 마세요. 업적 패널과 알림 레이어는 `KND_AchievementManager.panel_layer` 및 `popup_layer`로 조정할 수도 있습니다.
