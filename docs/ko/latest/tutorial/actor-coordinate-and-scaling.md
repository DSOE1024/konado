...
화면은 여러 등분으로 나뉩니다(2–5등분, 기본 5등분). 왼쪽에서 오른쪽으로 세며, 캐릭터는 `0/5`부터 `5/5` 사이에 배치할 수 있습니다. 예를 들어 `1/5`는 캐릭터가 화면 왼쪽에 있음을 의미합니다.
...
컴포넌트는 캐릭터를 담는 `Control` 노드의 캔버스 컨테이너를 가로 방향으로 `division`개의 블록으로 등분합니다(블록 수는 2–5로 제한). 블록 인덱스는 **왼쪽에서 오른쪽으로 증가**합니다(0은 가장 왼쪽, `division`은 가장 오른쪽).

`character_position`(캐릭터 위치 인덱스)은 `[0, division]`으로 제한되고 캐릭터의 **중심**은 해당 분할선에 맞춰집니다. 스탠딩 이미지가 가장자리에서 시각적으로 넘치는 것을 피하려면 일반적으로 `1`부터 `division - 1`까지를 권장합니다.
