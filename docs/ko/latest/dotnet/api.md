...
`ShotStart`, `ShotEnd`, `DialogueLineStart`, `DialogueLineEnd`, `CustomSignal`, `RuntimeFailed`는 표준 C# 이벤트입니다. 저장은 `SaveGame`, `LoadGame`, `DeleteSave`, `GetSaveInfo`, `GetAllSaveInfo`를 사용하고, 이력과 체크포인트는 `CanRollback`, `Rollback`, `GetExecutionHistory`, `ClearExecutionHistory`, `CreateCheckpoint`, `RestoreCheckpoint`를 사용합니다.
