...
var shot = i18n?.LoadLocalizedScript("res://dialogues/chapter.ks", "ko");
if (shot != null)
...
| `void SetShot(Resource shot)` / `void SetShot(KndShot shot)` | 현재 샷을 변경합니다. 래퍼를 직접 전달할 수 있습니다. |
...
| `DialogueLineStart(string nodeId)` | `dialogue_line_start(node_id)`에 연결됩니다. 현재 버전은 이전 줄 번호 대신 노드 ID를 사용합니다. |
...
Wrapper 클래스는 Konado GDScript 리소스를 감싸는 가벼운 C# 래퍼입니다. 기존 리소스를 감쌀 때는 스크립트 타입을 검증하고, 새 래퍼를 만들 때는 대응하는 GDScript 리소스를 생성합니다. Godot API가 기반 `Resource`를 명시적으로 요구할 때만 `SourceResource`를 사용하세요. `SetShot(KndShot)`에는 래퍼를 직접 전달할 수 있습니다.
...
| `ActorPosition` | `Vector2` | 캐릭터 위치입니다. 현재 버전은 그리드 기반 배치를 사용합니다. |
...
| `BackgroundName` | `string` | 전환할 배경 이름입니다. |
