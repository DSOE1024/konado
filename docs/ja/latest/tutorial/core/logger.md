...
エラー発生時、会話シーンは画面上にログウィンドウを重ねて表示し、エラー情報を示してゲーム実行を中断します。通常の警告は引き続きログファイルに記録されますが、実行時エラーとして表示されたり、ゲームを中断したりすることはありません。エラーオーバーレイを無効にする場合は、`KND_DialogueManager` の `enable_overlay_log` プロパティを `false` に設定してください。
