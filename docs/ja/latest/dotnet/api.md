...
`ShotStart`、`ShotEnd`、`DialogueLineStart`、`DialogueLineEnd`、`CustomSignal`、`RuntimeFailed` は標準 C# イベントです。セーブには `SaveGame`、`LoadGame`、`DeleteSave`、`GetSaveInfo`、`GetAllSaveInfo` を、履歴とチェックポイントには `CanRollback`、`Rollback`、`GetExecutionHistory`、`ClearExecutionHistory`、`CreateCheckpoint`、`RestoreCheckpoint` を使用します。
