...
A localized story may change translatable values only. Its control flow and stable instruction IDs must match the default story. Konado validates the complete overlay before installation, so an invalid variant cannot partially replace or corrupt active playback.
