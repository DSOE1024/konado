...
The screen is divided into equal sections (2–5, with 5 as the default). Counting from left to right, actors can be placed from `0/5` through `5/5`; for example, `1/5` places the actor on the left side of the screen.
...
The component horizontally divides the canvas container of the `Control` node that carries the actor into `division` blocks. The block count is clamped to 2–5, and indices increase **from left to right** (`0` is the far left and `division` is the far right).

`character_position` (the actor position index) is clamped to `[0, division]`, and the actor's **center** aligns with the corresponding division line. To avoid portraits visually overflowing at the edges, values from `1` through `division - 1` are normally preferable.
